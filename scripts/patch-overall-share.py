...
css_anchor = "  .rus-export-board.rus-export-state25 .rus-export-move{font-size:12px}\n"
overall_css = """  .rus-export-overall-board{display:flex;flex-direction:column;gap:10px}
...
"""
if '.rus-export-overall-board' not in s:
    if css_anchor not in s:
        raise SystemExit('Overall share CSS anchor not found')
    s = s.replace(css_anchor, css_anchor + overall_css, 1)

helper_anchor = "  async function rankingSource(el,w,h){\n"
helper = """  async function overallRankingSource(root,rows,w,h){
...
"""
if 'async function overallRankingSource(' not in s:
    if helper_anchor not in s:
        raise SystemExit('rankingSource anchor not found')
    s = s.replace(helper_anchor, helper + helper_anchor, 1)

old_start = """    const stateRoot=el.matches?.('.state25')?el:el.closest?.('.state25');
...
"""
new_start = """    const stateRoot=el.matches?.('.state25')?el:el.closest?.('.state25');
    const stateRows=stateRoot?[...stateRoot.querySelectorAll('.state25-row')]:[];
...
elif "const smallRoot=el.matches?.('.small-school-section')" not in s:
    raise SystemExit('Could not find rankingSource row-selection block')

p.write_text(s, encoding='utf-8')

q = Path('rankings.html')
t = q.read_text(encoding='utf-8')
t = re.sub(r'share-graphic\.js\?v=[^\"\']+', 'share-graphic.js?v=20260816-overall-featured-top3-v1', t)
