...
const weeks=ordered.map(week=>{
  ...
  const playerKey=p=>p?.id||`${canon(p?.team)}|${compact(p?.number)}|${compact(p?.name)}`,
    statewideMvpKey=playerKey(overall[0]),
    ruralAwardPool=rural.filter(p=>playerKey(p)!==statewideMvpKey),
    ruralMvpKey=playerKey(ruralAwardPool[0]),
    offenseAwardPool=offense.filter(p=>playerKey(p)!==statewideMvpKey),
    statewideOffenseKey=playerKey(offenseAwardPool[0]),
    defenseAwardPool=defense.filter(p=>playerKey(p)!==statewideMvpKey),
    ruralOffenseAwardPool=ruralOffense.filter(p=>playerKey(p)!==statewideMvpKey&&playerKey(p)!==ruralMvpKey&&playerKey(p)!==statewideOffenseKey),
    ruralDefenseAwardPool=ruralDefense.filter(p=>playerKey(p)!==ruralMvpKey);
  const majorAwardKeys=new Set([overall[0],offenseAwardPool[0],defenseAwardPool[0],ruralAwardPool[0],ruralOffenseAwardPool[0],ruralDefenseAwardPool[0]].filter(Boolean).map(playerKey));
