...
const weeks=ordered.map(week=>{
  ...
  const players=[...week.players.values()].map(p=>{p.rawOverall=Math.max(p.offense,p.defense)+Math.min(p.offense,p.defense)*.35+p.winBonus;p.weight=classWeight[p.classification]||1;p.weightedOverall=p.rawOverall*p.weight;p.weightedOffense=p.offense*p.weight;p.weightedDefense=p.defense*p.weight;return p}).filter(p=>p.rawOverall>0&&classOrder.includes(p.classification)&&!missingKeys.has(canon(p.team)));
  const overall=sorted(players,p=>p.weightedOverall),offense=sorted(players,p=>p.weightedOffense,p=>p.offense>0),defense=sorted(players,p=>p.weightedDefense,p=>p.defense>0),rural=sorted(players,p=>p.rawOverall,p=>ruralClasses.has(p.classification)),ruralOffense=sorted(players,p=>p.offense,p=>ruralClasses.has(p.classification)&&p.offense>0),ruralDefense=sorted(players,p=>p.defense,p=>ruralClasses.has(p.classification)&&p.defense>0);
  ...
  const classAwards=classOrder.map(classification=>{const rows=sorted(players,p=>p.rawOverall,p=>p.classification===classification);return{classification,label:classification==='8P'?'8-Player':classification,winner:rows[0]?playerView(rows[0],rows[0].rawOverall,'RUS weekly score'):null,contenders:rows.slice(0,5).map(p=>playerView(p,p.rawOverall,'RUS weekly score'))}});
