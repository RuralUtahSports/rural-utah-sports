...
const weeks=ordered.map(week=>{
  ...
  const overall=sorted(players,p=>p.weightedOverall),offense=sorted(players,p=>p.weightedOffense,p=>p.offense>0),defense=sorted(players,p=>p.weightedDefense,p=>p.defense>0),rural=sorted(players,p=>p.rawOverall,p=>ruralClasses.has(p.classification));
  ...
  return{week:weekNumber,startDate:isoDay(week.start),endDate:isoDay(week.start+6*DAY),lockDate,locked:shouldLock,lockedAt:shouldLock?new Date().toISOString():null,gameCount:week.gameKeys.size,teamGames:week.teamGames,playersReported:players.length,coverage:{teamsInCompletedGames:coverage.teams.size,teamsReportingStats:Math.max(0,coverage.teams.size-missingTeams.size),missingTeams:[...missingTeams].sort()},awards:{overall:overall[0]?playerView(overall[0],overall[0].weightedOverall,'Weighted Utah score'):null,offense:offense[0]?playerView(offense[0],offense[0].weightedOffense,'Weighted offense score'):null,defense:defense[0]?playerView(defense[0],defense[0].weightedDefense,'Weighted defense score'):null,rural:rural[0]?playerView(rural[0],rural[0].rawOverall,'Rural weekly score'):null},contenders:{overall:overall.slice(0,10).map(p=>playerView(p,p.weightedOverall,'Weighted Utah score')),offense:offense.slice(0,10).map(p=>playerView(p,p.weightedOffense,'Weighted offense score')),defense:defense.slice(0,10).map(p=>playerView(p,p.weightedDefense,'Weighted defense score')),rural:rural.slice(0,10).map(p=>playerView(p,p.rawOverall,'Rural weekly score'))},classes:classAwards};
});
