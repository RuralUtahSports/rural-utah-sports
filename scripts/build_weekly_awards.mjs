...
const weeks=ordered.map(week=>{
  ...
  const playerKey=p=>p?.id||`${canon(p?.team)}|${compact(p?.number)}|${compact(p?.name)}`,statewideMvpKey=playerKey(overall[0]),ruralAwardPool=rural.filter(p=>playerKey(p)!==statewideMvpKey),ruralMvpKey=playerKey(ruralAwardPool[0]),offenseAwardPool=offense.filter(p=>playerKey(p)!==statewideMvpKey),defenseAwardPool=defense.filter(p=>playerKey(p)!==statewideMvpKey),ruralOffenseAwardPool=ruralOffense.filter(p=>playerKey(p)!==statewideMvpKey&&playerKey(p)!==ruralMvpKey),ruralDefenseAwardPool=ruralDefense.filter(p=>playerKey(p)!==ruralMvpKey);
