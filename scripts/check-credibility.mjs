...
const sw=fs.readFileSync('sw.js','utf8');if(/addEventListener\(['\"]fetch['\"]/.test(sw))fail('retired service worker must not intercept credibility assets');for(const n of ['registration.unregister','rus-site-'])if(!sw.includes(n))fail(`service worker retirement flow missing ${n}`);
