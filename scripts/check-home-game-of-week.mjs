...
for(const token of ['Game of the Week','RURAL_CLASSES','3A','2A','1A','8P','game-of-the-week-2026.json','home-week-data.json','RUS Projection','Open Game Center','rus-gotw-sponsor','Presented by']){
  if(!js.includes(token))fail(`home-game-of-week.js missing ${token}`);
}
for(const token of ['home-game-of-week.js?v=20260819-gotw1','data-rus-home-gotw','loadHomeGotw']){
...
if(!process.exitCode)console.log(`Game of the Week checks passed with ${week.games.length} weekly games available; sponsor slot is inactive and ready.`);
