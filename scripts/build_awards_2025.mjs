...
const statewideMvp=rank(all,p=>p.mvpScore,40);
const statewideMvpKey=playerKey(statewideMvp[0]);
const ruralMvp=rank(ruralRows.filter(p=>playerKey(p)!==statewideMvpKey),p=>p.ruralMvpScore,40);
