...
// Preserve the original 2025 overall/MVP model. Position teams below use the same
// shared scoring core as the live 2026 All-State/All-Region watch.
...
const players=new Map();for(const [teamName,t] of Object.entries(rosterData.teams||{})){const m=meta.get(canon(teamName));if(!m)continue;const rosterById=new Map((t.roster||[]).map(p=>[p.playerId,p]));for(const sec of t.stats||[])for(const row of sec.rows||[]){const id=row.playerId||`${teamName}|${row.number}|${row.name}`;if(!players.has(id)){const rp=rosterById.get(row.playerId)||{};players.set(id,{id,team:m.team,name:row.name,number:row.number,classYear:rp.class||'',rawPosition:rp.position||'',position:normPos(rp.position),classification:m.classification,region:m.region,offense:0,defense:0,lines:[]})}const p=players.get(id),pts=legacyStatPoints(sec.category,row.values);if(side(sec.category)==='Defense')p.defense+=pts;else if(side(sec.category)==='Offense')p.offense+=pts;p.lines.push({category:sec.category,points:+pts.toFixed(2),values:row.values})}}
...
const allState={};for(const cls of ['6A','5A','4A','3A','2A','1A','8P']){const rows=all.filter(p=>(p.classification==='8-PLAYER'?'8P':p.classification)===cls);allState[cls]={overall:rank(rows,p=>p.rawScore,40),positions:byPosition(rows,p=>p.positionScore,10)}}
const regionMap={};for(const p of all){const key=`${p.classification}|${p.region}`;(regionMap[key]??=[]).push(p)}const allRegion={};for(const [key,rows] of Object.entries(regionMap)){allRegion[key]={classification:rows[0].classification,region:rows[0].region,overall:rank(rows,p=>p.rawScore,30),positions:byPosition(rows,p=>p.positionScore,8)}}
const out={season:SEASON,generatedAt:new Date().toISOString(),method:{note:`2025 RUS awards model built from reported Deseret player stats, RUS 2025 team records and no estimated missing stats. All-State, All-Region and All-Utah position teams use the same shared position formulas as the live 2026 watch (${scoring.VERSION}): QB/RB/WR/TE/ATH count passing + rushing + receiving only, defensive stats are excluded from those offensive positions, and run-first QBs receive enhanced rushing credit.`,scoringVersion:scoring.VERSION,classWeight},coverage:{teamsInArchive:Object.keys(rosterData.teams||{}).length,teamsWithStats:Object.values(rosterData.teams||{}).filter(x=>x.stats?.some(s=>s.rows?.length)).length,rankedPlayers:all.length},mvp:{statewide:rank(all,p=>p.mvpScore,40),offense:rank(all.filter(p=>p.offense>0),p=>p.offense+p.teamBonus,40),defense:rank(all.filter(p=>p.defense>0),p=>p.defense+p.teamBonus,40),rural:rank(all.filter(p=>rural.has(p.classification)),p=>p.ruralMvpScore,40),bigSchool:rank(all.filter(p=>big.has(p.classification)),p=>p.bigMvpScore,40)},allUtah:{overall:rank(all,p=>p.allUtahScore,75),positions:byPosition(all,p=>p.allUtahPositionScore,12)},allState,allRegion};
