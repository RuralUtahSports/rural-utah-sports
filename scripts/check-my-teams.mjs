...
const required=['my-teams.html','my-teams-dashboard-v4.js','teams-data.json','standings-2026.json','elo-summary.json','weekly-simulation.json','rankings-current-2026.json','deseret-rosters-stats-2026.json','share-preview-map.json'];
...
for(const token of ['my-teams-dashboard-v4.js?v=20260818-mt4','data-rus-my-teams-dashboard','without needing a full-season schedule']){
  if(!html.includes(token)) throw new Error(`My Teams page is missing ${token}`);
}

const js=fs.readFileSync('my-teams-dashboard-v4.js','utf8');
