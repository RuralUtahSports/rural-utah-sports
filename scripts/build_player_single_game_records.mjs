...
async function fetchHtml(url,tries=3){let last;for(let attempt=1;attempt<=tries;attempt++){try{const r=await fetch(url,{headers:{'user-agent':'Mozilla/5.0 (compatible; RuralUtahSports/1.0; +https://ruralutahsports.com/)'},redirect:'follow',signal:AbortSignal.timeout(25000)});if(r.status===404)return null;if(!r.ok)throw new Error(`${r.status} ${r.statusText}`);return await r.text()}catch(e){last=e;if(attempt<tries)await sleep(300*attempt)}}throw last}
...
function tableTeam(html,index,pageTeamNames){const prior=text(String(html||'').slice(Math.max(0,index-2500),index));const matches=pageTeamNames.filter(t=>normalize(prior).includes(normalize(t)));return matches.at(-1)||null}
