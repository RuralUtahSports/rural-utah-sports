...
const coverageNote='Single-game player records use Deseret News game stat tables from 2009 to present. For 2001–2008, cumulative season totals shown on game pages are excluded; only game-level touchdowns/kicking values and separately verified record lines are used. Reporting gaps remain, so this is not a complete all-history UHSAA record book.';
const byTeam=new Map();for(const p of performances){if(!byTeam.has(p.team))byTeam.set(p.team,[]);byTeam.get(p.team).push(p)}for(const team of teams){const ps=byTeam.get(team)||[],categories=[];for(const key of Object.keys(CATEGORY_DEFS)){const rows=ps.filter(p=>Number(p.stats[key])>0).map(p=>entryFromPerformance(p,key));if(rows.length)categories.push(categoryPayload(key,rows,TEAM_LIMIT))}const payload={team,range:`${START_SEASON}–present`,source:'Deseret News',coverageNote,categories};fs.writeFileSync(path.join(OUT_DIR,'by-team',`${slug(team)}.json`),JSON.stringify(payload,null,2)+'\n')}
