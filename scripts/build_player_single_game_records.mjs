...
const coverageNote='Single-game player records combine available Deseret News game-stat tables from 2009 to present with official UHSAA record-book single-game entries when the school is identified. For 2001–2008, Deseret pages contribute only explicitly game-level stats; cumulative season tables are not treated as games. Reporting gaps remain, so this is not a complete all-history UHSAA record book.';
...
const statewidePayload={range:`${START_SEASON}–present`,source:'Deseret News and UHSAA record book',coverageNote,categories:statewide};fs.writeFileSync(path.join(OUT_DIR,'statewide.json'),JSON.stringify(statewidePayload,null,2)+'\n');const summary={updatedAt:new Date().toISOString(),startSeason:START_SEASON,structuredSingleGameStartSeason:STRUCTURED_SINGLE_GAME_START_SEASON,endSeason:END_SEASON,uniqueGamePages:discovery.games.length,gamePageFailures:failures,performances:performances.length,teams:teams.length,teamsWithPerformances:[...byTeam.keys()].length,categories:statewide.length,seasonDiscovery:discovery.seasonCounts,manualVerifiedEntries:MANUAL.length,uhsaaSingleGameEntries:uhsaaEntries.length};fs.writeFileSync(path.join(OUT_DIR,'index.json'),JSON.stringify(summary,null,2)+'\n');console.log(summary);if(discovery.games.length<5000)throw new Error(`Historical discovery unexpectedly small: ${discovery.games.length}`);if(performances.length<10000)throw new Error(`Performance output unexpectedly small: ${performances.length}`)}
export { gameLinks, MANUAL, mergeManual, parseGame, loadUhsaaSingleGameEntries };
