...
function loadUhsaaSingleGameEntries(){const manifestPath='uhsaa-records-expanded.json';if(!fs.existsSync(manifestPath))return[];const manifest=JSON.parse(fs.readFileSync(manifestPath,'utf8'));const files=Array.isArray(manifest.parts)?manifest.parts:[];const out=[];for(const file of files){if(!fs.existsSync(file))continue;const part=JSON.parse(fs.readFileSync(file,'utf8'));for(const category of part.categories||[]){if(!/game/i.test(category.title||''))continue;const base=clean(category.title).replace(/\s*[-–]\s*Game\s*$/i,'').trim();const key=Object.keys(CATEGORY_DEFS).find(k=>normalize(CATEGORY_DEFS[k].label)===normalize(base));if(!key)continue;for(const e of category.entries||[]){const team=canonicalTeam(e.school);if(!team||!e.name||!Number.isFinite(Number(e.value)))continue;const detail=clean(e.detail);const year=detail.match(/\b(19|20)\d{2}\b/)?.[0]||'';const dateMatch=detail.match(/(\d{1,2})-(\d{1,2})-(\d{4})/);const date=dateMatch?`${dateMatch[3]}-${String(dateMatch[1]).padStart(2,'0')}-${String(dateMatch[2]).padStart(2,'0')}`:'';const opponent=detail.replace(/\b\d{1,2}-\d{1,2}-\d{4}\b\s*/,'').replace(/^vs?\\.?\s*|^at\s*/i,'').replace(/,?\s*(OT|2OT|3OT)$/i,'').trim();const score=historicalScore(team,date,opponent);out.push({key,team,player:e.name,season:Number(year)||null,date,opponent,teamScore:score.teamScore??null,opponentScore:score.opponentScore??null,value:Number(e.value),gameId:'uhsaa-'+slug(team)+'-'+slug(e.name)+'-'+year+'-'+key,gameUrl:'',source:'UHSAA record book'});}}}return out}
