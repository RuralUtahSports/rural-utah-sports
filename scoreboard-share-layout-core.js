(()=>{
  ...
  function loadShareContext(){
    ...
    shareContextPromise=(async()=>{
      const stamp=Date.now(),get=file=>fetch(`${file}?v=${stamp}`,{cache:'no-store'}).then(r=>r.ok?r.json():null).catch(()=>null),[standings,classArchive,stateData]=await Promise.all([get('standings-2026.json'),get('rankings-history-2026.json'),get('state-top25-history-2026.json')]),records=new Map(),classRanks=new Map(),stateRanks=new Map();
      ...
      const classSnapshots=classArchive?.snapshots||[],classData=classSnapshots.at(-1)||classArchive;
