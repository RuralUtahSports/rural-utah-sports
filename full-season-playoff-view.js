(() => {
  const F = (window.RUSFullSeason = window.RUSFullSeason || {}),
    order = { "6A": 0, "5A": 1, "4A": 2, "3A": 3, "2A": 4, "1A": 5, "8P": 6 },
    h = (v) =>
      String(v ?? "")
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
  ...
  const className = (c) => (c === "8P" ? "8-Player" : c);
