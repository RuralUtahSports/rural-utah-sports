(()=>{
  ...
  const ORANGE='#F14D07',MAX_GAMES=32,COMPACT_FORMAT_LIMIT=8,GRID_COLUMNS=4,GRID_ROWS=8;
  ...
  function visibleGames(){
    return [...document.querySelectorAll('#board .game')].map((el,index)=>{
      ...
      const teamData=rows.slice(0,2).map(row=>{const nameEl=row.querySelector('.team-name'),img=row.querySelector('.team-logo'),actual=row.querySelector('.actual b')?.textContent?.trim()||'—',projected=row.dataset.projectedScore||'';return{name:nameEl?.textContent?.trim()||'',meta:row.querySelector('.team-meta')?.textContent?.trim()||'',score:actual==='—'&&projected?projected:actual,projected:actual==='—'&&!!projected,color:nameEl?.style.getPropertyValue('--team-bg')||row.style.getPropertyValue('--team-wash')||'#252525',logo:img?.currentSrc||img?.src||'',winner:row.classList.contains('rus-winner')||row.classList.contains('winner')}});
  ...
  async function buildGrid32Canvas(items,classValue='ALL'){
    const width=2160,height=2700,margin=42,gap=14,gridTop=190,footerH=54,gridBottom=height-footerH-20,cardW=(width-margin*2-gap*(GRID_COLUMNS-1))/GRID_COLUMNS,cardH=(gridBottom-gridTop-gap*(GRID_ROWS-1))/GRID_ROWS;
    ...
    const title=titleFor(items,classValue);c.textAlign='left';c.textBaseline='alphabetic';c.fillStyle='#fff';fitCanvasText(c,title,width-margin*2,58,34,1000);c.fillText(title,margin,78);c.fillStyle=ORANGE;c.font='1000 25px Arial,Helvetica,sans-serif';c.fillText('RURAL UTAH SPORTS',margin,116);c.fillStyle='#888';c.font='900 18px Arial,Helvetica,sans-serif';c.fillText('4 COLUMNS • 8 ROWS • UP TO 32 GAMES',margin,151);
    const logos=await loadShareLogos(),limited=items.slice(0,MAX_GAMES),images=await Promise.all(limited.flatMap(item=>item.teams.slice(0,2).map(team=>loadCanvasImage(shareLogoFor(team.name,logos)||team.logo))));
    limited.forEach((item,index)=>{const col=index%GRID_COLUMNS,row=Math.floor(index/GRID_COLUMNS),x=margin+col*(cardW+gap),y=gridTop+row*(cardH+gap),topH=46,footH=36,teamH=(cardH-topH-footH)/2;
      ...
      const status=String(item.status||'UPCOMING').toUpperCase(),statusFill=/FINAL/.test(status)?ORANGE:/LIVE|Q[1-4]|HALF|OT/.test(status)?'#FFD54A':'#252525';c.fillStyle=statusFill;canvasRoundRect(c,x+11,y+10,Math.min(116,Math.max(66,status.length*8+22)),25,12);c.fill();c.fillStyle=statusFill==='#252525'?'#bbb':'#080808';c.font='1000 12px Arial,Helvetica,sans-serif';c.textAlign='center';c.textBaseline='middle';const badgeW=Math.min(116,Math.max(66,status.length*8+22));c.fillText(status,x+11+badgeW/2,y+22.5);c.textAlign='right';c.fillStyle='#888';fitCanvasText(c,item.date,cardW-badgeW-42,12,8,900);c.fillText(item.date,x+cardW-11,y+26);
      item.teams.slice(0,2).forEach((team,teamIndex)=>{const ty=y+topH+teamIndex*teamH,color=safeCanvasColor(team.color),logoSize=Math.min(60,teamH-22),logoX=x+14,logoY=ty+(teamH-logoSize)/2,nameX=x+84,scoreX=x+cardW-25;c.fillStyle=teamIndex?'#080808':'#0c0c0c';c.fillRect(x,ty,cardW,teamH);c.save();c.globalAlpha=.22;c.fillStyle=color;c.fillRect(x,ty,cardW*.62,teamH);c.restore();c.fillStyle=color;c.fillRect(x,ty,7,teamH);const img=images[index*2+teamIndex];if(img)drawCanvasContain(c,img,logoX,logoY,logoSize,logoSize);c.textAlign='left';c.textBaseline='alphabetic';c.fillStyle='#fff';fitCanvasText(c,team.name,cardW-176,23,12,1000);c.fillText(team.name,nameX,ty+teamH*.48);c.fillStyle='#8b8b8b';fitCanvasText(c,team.meta,cardW-176,12,8,900);c.fillText(team.meta,nameX,ty+teamH*.73);c.textAlign='center';c.fillStyle='#777';c.font='900 9px Arial,Helvetica,sans-serif';c.fillText(team.projected?'PROJ':'SCORE',scoreX,ty+teamH*.34);c.fillStyle=team.winner?'#73D977':team.projected?'#bbb':ORANGE;c.font='1000 34px Arial,Helvetica,sans-serif';c.fillText(team.score||'—',scoreX,ty+teamH*.69)});
    ...
    c.textAlign='left';c.textBaseline='alphabetic';c.fillStyle='#777';c.font='900 18px Arial,Helvetica,sans-serif';c.fillText('ruralutahsports.github.io',margin,height-20);c.textAlign='right';c.fillText(`${limited.length} GAME${limited.length===1?'':'S'}`,width-margin,height-20);return canvas;
  }

  async function deliverCanvas(canvas,title='Rural Utah Sports Scoreboard'){
    const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png',1));if(!blob)throw new Error('Could not encode scoreboard graphic.');const file=new File([blob],`rural-utah-sports-scoreboard-${Date.now()}.png`,{type:'image/png'});
    if(navigator.share&&navigator.canShare?.({files:[file]})){try{await navigator.share({files:[file],title});return}catch(e){if(e?.name==='AbortError')return}}
  ...
  async function createGraphic(format,items,classValue='ALL'){
    if(format==='grid32'){const canvas=await buildGrid32Canvas(items,classValue);await deliverCanvas(canvas,'Rural Utah Sports 32-Game Scoreboard');return}
  ...
  async function openModal(){
    ...
    modal.querySelectorAll('[data-f]').forEach(btn=>btn.onclick=async()=>{const selected=boxes.filter(b=>b.checked).map(b=>games[Number(b.value)]).filter(Boolean);if(!selected.length){alert('Select at least one game.');return}if(btn.dataset.f!=='grid32'&&selected.length>COMPACT_FORMAT_LIMIT){alert('Instagram and X formats support up to 8 games. Select 8 or fewer, or use the 32-game scoreboard option.');return}const old=btn.innerHTML;btn.disabled=true;btn.textContent='Creating…';try{await createGraphic(btn.dataset.f,selected,classFilter.value);modal.remove()}catch(e){console.error(e);btn.disabled=false;btn.innerHTML=old;alert('Could not create the scoreboard graphic. Please try again.')}});
  }

  window.RUSScoreboardShare={openModal};
  function init(){addStyles();const replace=()=>{const buttons=[...document.querySelectorAll('.rus-share-float,#shareScoreboardGrid')];buttons.forEach(b=>b.onclick=openModal);return buttons.length>0};if(!replace()){let n=0;const t=setInterval(()=>{if(replace()||++n>40)clearInterval(t)},100)}}
