(()=>{
  ...
  function cloneGame(item,logos,includePredictedScores=false){
    ...
    [item.away,item.home].forEach((team,i)=>{
      ...
      const score=teamRows[i].querySelector('.actual');if(teamRows[i]&&item.teams[i]?.projected&&!includePredictedScores)score?.remove();else if(score&&item.teams[i]?.projected){const value=score.querySelector('b');if(value)value.textContent=item.teams[i].score||'—'}
  ...
  async function createPagedGraphics(format,items,classValue='ALL',onProgress=()=>{},includePredictedScores=false){
  ...
  async function buildBoard(format,items,classValue='ALL',includePredictedScores=false){
    ...
    items.forEach(item=>grid.appendChild(cloneGame(item,logos,includePredictedScores)));
  ...
  async function createGraphic(format,items,classValue='ALL',onProgress=()=>{},includePredictedScores=false){
    if(format==='all-pages'||format==='carousel10'){await createPagedGraphics(format,items,classValue,onProgress,includePredictedScores);return}
    if(format==='grid32'){const canvas=await buildScoreboardCanvas(items.slice(0,MAX_GAMES),classValue,{columns:GRID_COLUMNS,rows:GRID_ROWS,includePredictedScores,descriptor:'4 COLUMNS • 8 ROWS • UP TO 32 GAMES'});await deliverCanvas(canvas,'Rural Utah Sports 32-Game Scoreboard',`rural-utah-sports-scoreboard-${Date.now()}.png`);return}
    await loadCanvas();const {board,w,h}=await buildBoard(format,items,classValue,includePredictedScores);
    try{
  ...
  async function openModal(){
    ...
    const modal=document.createElement('div');modal.className='rus-share-modal';modal.innerHTML=`
      <div class="rus-share-sheet">
        ...
        <label class="rus-sb-prediction-toggle"><input type="checkbox" class="rus-sb-include-predictions"><span>Include predicted scores<small>Off by default. Actual live/final scores and the RUS line remain visible.</small></span></label>
    ...
    modal.querySelectorAll('[data-f]').forEach(btn=>btn.onclick=async()=>{const format=btn.dataset.f,paged=format==='all-pages'||format==='carousel10',source=(paged?shownBoxes():boxes.filter(b=>b.checked)).map(b=>games[Number(b.value)]).filter(Boolean),includePredictedScores=modal.querySelector('.rus-sb-include-predictions')?.checked===true;if(!source.length){alert(paged?'No games are shown for this classification.':'Select at least one game.');return}if(!paged&&format!=='grid32'&&source.length>COMPACT_FORMAT_LIMIT){alert('Instagram and X single-image formats support up to 8 games. Select 8 or fewer, or use an All Games option.');return}const old=btn.innerHTML;btn.disabled=true;btn.textContent='Creating…';try{await createGraphic(format,source,classFilter.value,(current,total)=>{btn.textContent=`Creating ${current} of ${total}…`},includePredictedScores);modal.remove()}catch(e){console.error(e);btn.disabled=false;btn.innerHTML=old;alert('Could not create the scoreboard graphic. Please try again.')}});
  }
