(()=>{
  ...
  const canon=v=>({'CEDAR':'CEDAR CITY','MONUMENT VAL':'MONUMENT VALLEY','GUNNISON':'GUNNISON VALLEY','GRAND COUNTY':'GRAND'}[norm(v)]||norm(v));
  ...
  function loadImage(src){if(!src)return Promise.resolve(null);return new Promise(resolve=>{const img=new Image();let done=false,timer=null;const finish=v=>{if(done)return;done=true;if(timer)clearTimeout(timer);resolve(v)};try{const url=new URL(src,location.href);if(url.origin!==location.origin)img.crossOrigin='anonymous';img.onload=()=>finish(img);img.onerror=()=>finish(null);img.decoding='async';img.src=url.href;timer=setTimeout(()=>finish(null),2500)}catch{resolve(null)}})}
  async function fetchJson(file){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),3500);try{const r=await fetch(`${file}?v=${Date.now()}`,{cache:'no-store',signal:controller.signal});return r.ok?await r.json():null}catch{return null}finally{clearTimeout(timer)}}

  let contextPromise=null;
  function loadContext(){
    if(contextPromise)return contextPromise;
    contextPromise=(async()=>{
      const [standings,classData,stateData]=await Promise.all([fetchJson('standings-2026.json'),fetchJson('rankings-current-2026.json'),fetchJson('state-top25-history-2026.json')]);
  ...
  function parseGame(el,context){
    ...
    const teams=rows.map(row=>{
      ...
      return{name,displayName:stateRank?`#${stateRank.rank} ${name}`:name,meta:meta.join(' • '),score:row.querySelector('.actual b')?.textContent?.trim()||'—',hasScore:(row.querySelector('.actual b')?.textContent?.trim()||'—')!=='—',winner:row.classList.contains('winner')||row.classList.contains('rus-winner'),logo:window.RUSSchoolAssets?.logoUrl?.(name)||img?.currentSrc||img?.src||'',color:teamColor};
    });
  ...
  async function renderCompact(format,elements,classValue='ALL'){
    ...
    const images=await Promise.all(items.flatMap(item=>item.teams.map(team=>loadImage(team.logo))));
    items.forEach((item,index)=>{
      ...
      item.teams.forEach((team,ti)=>{const ty=y+topH+ti*teamH,logoSize=Math.max(22,Math.min(teamH-14,format==='story'?54:46)),logoX=x+10,logoY=ty+(teamH-logoSize)/2,showScore=team.hasScore,scoreW=showScore?Math.max(64,cardW*.19):0,nameX=logoX+logoSize+9,textRight=x+cardW-scoreW-10,textW=Math.max(55,textRight-nameX);c.fillStyle=ti?'#080808':'#0c0c0c';c.fillRect(x,ty,cardW,teamH);c.save();c.globalAlpha=.22;c.fillStyle=team.color;c.fillRect(x,ty,cardW-scoreW,teamH);c.restore();c.fillStyle=team.color;c.fillRect(x,ty,6,teamH);const img=images[index*2+ti];if(img)drawContain(c,img,logoX,logoY,logoSize,logoSize);c.fillStyle='#fff';c.textAlign='left';c.textBaseline='alphabetic';fitText(c,team.displayName,textW,Math.max(12,Math.min(20,teamH*.27)),10,1000);c.fillText(team.displayName,nameX,ty+teamH*.48);c.fillStyle='#999';fitText(c,team.meta,textW,Math.max(8,Math.min(10,teamH*.15)),7,900);c.fillText(team.meta,nameX,ty+teamH*.72);if(showScore){const sx=x+cardW-scoreW/2;c.textAlign='center';c.fillStyle='#777';c.font=`900 ${Math.max(7,Math.min(9,teamH*.12))}px Arial,Helvetica,sans-serif`;c.fillText('SCORE',sx,ty+teamH*.34);c.fillStyle=team.winner?'#73D977':ORANGE;c.font=`1000 ${Math.max(22,Math.min(34,teamH*.43))}px Arial,Helvetica,sans-serif`;c.fillText(team.score,sx,ty+teamH*.7)}});
  ...
  const loadCore=()=>{if(document.querySelector('script[data-rus-scoreboard-share-core="1"]'))return;const core=document.createElement('script');core.src='scoreboard-share-layout-core.js?v=20260820-native-compact1';core.defer=true;core.dataset.rusScoreboardShareCore='1';document.head.appendChild(core)};
  loadCore();
})();