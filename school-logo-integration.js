...
function start(){
  ...
  const enhance=root=>{
    ...
    nodes(root,'.team-card:not([data-rus-school-logo])').forEach(card=>{const name=card.querySelector('.team-name')?.textContent?.trim(),content=card.querySelector('.team-card-content');if(!name||!content)return;const url=A.logoUrl?.(name)||'';if(!url)return;card.dataset.rusSchoolLogo='1';const wrap=document.createElement('div');wrap.className='rus-card-logo-wrap';const disc=A.logoDisc?.(name);if(disc){wrap.classList.add('has-logo-disc');wrap.style.setProperty('--rus-logo-disc-color',disc.color);wrap.style.setProperty('--rus-logo-disc-size',`${disc.size||72}px`)}wrap.appendChild(addImg(name,'rus-card-logo'));content.insertBefore(wrap,content.firstChild)});
