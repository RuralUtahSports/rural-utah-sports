...
function start(){
  ...
  const replaceExisting=root=>nodes(root,'img').forEach(img=>{
    ...
    const url=A.logoUrl?.(team)||'';if(url&&img.getAttribute('src')!==url){img.src=url;img.srcset='';img.dataset.rusCustomLogo='1'}const disc=A.logoDisc?.(team);if(disc&&!img.closest('.rus-card-logo-wrap')){img.classList.add('rus-logo-disc');img.style.setProperty('--rus-logo-disc-color',disc.color);}
  });
  const dedupeScoreboardLiveStatus=root=>{
    if((location.pathname.split('/').pop()||'').toLowerCase()!=='scoreboard.html')return;
    nodes(root,'.status.live').forEach(el=>{
      const tokens=String(el.textContent||'').toUpperCase().replace(/[•·]/g,' ').match(/[A-Z0-9:-]+/g)||[];
      if(tokens.length>1&&tokens.every(token=>token==='LIVE'))el.textContent='LIVE';
    });
  };
  const style=document.createElement('style');style.id='rus-school-logo-style';style.textContent='.rus-card-logo-wrap{height:82px;display:flex;align-items:center;justify-content:center;margin:-2px 0 10px;overflow:visible;position:relative}.rus-card-logo-wrap.has-logo-disc::before{content:'';position:absolute;width:var(--rus-logo-disc-size,72px);height:var(--rus-logo-disc-size,72px);border-radius:50%;background:var(--rus-logo-disc-color,#fff);z-index:0}.rus-card-logo{max-width:78px;max-height:78px;object-fit:contain;position:relative;z-index:1}.rus-logo-disc{background:var(--rus-logo-disc-color,#fff)!important;border-radius:50%!important;padding:2px!important}.rus-card-logo[data-team="Ridgeline"]{width:150px;height:78px;max-width:100%;max-height:78px;object-fit:contain;object-position:center;display:block}.rus-ranking-school-logo{width:34px;height:34px;object-fit:contain;flex:0 0 34px;margin-right:8px}.rus-standings-school-logo{width:30px;height:30px;object-fit:contain;flex:0 0 30px;margin-right:7px}.rank-card{border-radius:12px!important;background:#0b0b0b!important}.rank-card .rank-head{padding:13px 16px!important;background:linear-gradient(180deg,#171717,#101010)!important}.rank-card .rank-head h2{font-size:21px!important;letter-spacing:.5px}.rank-card .rank-row{grid-template-columns:44px minmax(0,1fr) auto!important;gap:8px!important;min-height:56px!important;padding:8px 12px 8px 8px!important;border-left-width:4px!important;background:linear-gradient(90deg,var(--team-tint,rgba(255,255,255,.04)) 0%,rgba(0,0,0,0) 46%)!important}.rank-card .rank-row:hover{background:linear-gradient(90deg,var(--team-tint-strong,rgba(255,255,255,.08)) 0%,rgba(0,0,0,0) 58%)!important}.rank-card .rank-num{width:32px!important;height:32px!important;font-size:14px!important}.rank-card .team-link{gap:0!important;min-width:0!important}.rank-card .rus-ranking-school-logo{width:32px;height:32px;flex-basis:32px;margin-right:9px}.rank-card .team-pill{background:transparent!important;color:#fff!important;box-shadow:none!important;padding:0!important;min-height:0!important;border-radius:0!important;font-size:13px!important;letter-spacing:.2px;white-space:normal}.rank-card .team-class{font-size:9px!important;color:#777!important;background:#171717;border:1px solid #2b2b2b;border-radius:999px;padding:4px 7px}@media(max-width:700px){.rus-ranking-school-logo{width:30px;height:30px;flex-basis:30px}.rus-standings-school-logo{width:27px;height:27px;flex-basis:27px}.rank-card .rank-head{padding:11px 12px!important}.rank-card .rank-head h2{font-size:19px!important}.rank-card .rank-row{grid-template-columns:38px minmax(0,1fr)!important;min-height:50px!important;padding:6px 9px 6px 7px!important;gap:5px!important}.rank-card .rank-num{width:29px!important;height:29px!important;font-size:13px!important}.rank-card .rus-ranking-school-logo{width:28px;height:28px;flex-basis:28px;margin-right:8px}.rank-card .team-pill{font-size:12px!important}.rank-card .team-class{display:none!important}}';document.head.appendChild(style);
  const addImg=(team,cls)=>{const img=document.createElement('img');img.className=cls;img.alt=`${team} logo`;img.dataset.team=team;img.loading='lazy';img.decoding='async';img.src=A.logoUrl?.(team)||'';const disc=A.logoDisc?.(team);if(disc&&cls!=='rus-card-logo'){img.classList.add('rus-logo-disc');img.style.setProperty('--rus-logo-disc-color',disc.color)}return img};
  const enhance=root=>{
    dedupeScoreboardLiveStatus(root);
    replaceExisting(root);
    nodes(root,'.team-card:not([data-rus-school-logo])').forEach(card=>{const existing=card.querySelector('.rus-card-logo-wrap');if(existing){card.dataset.rusSchoolLogo='1';return}const name=card.querySelector('.team-name')?.textContent?.trim(),content=card.querySelector('.team-card-content');if(!name||!content)return;const url=A.logoUrl?.(name)||'';if(!url)return;card.dataset.rusSchoolLogo='1';const wrap=document.createElement('div');wrap.className='rus-card-logo-wrap';const disc=A.logoDisc?.(name);if(disc){wrap.classList.add('has-logo-disc');wrap.style.setProperty('--rus-logo-disc-color',disc.color);wrap.style.setProperty('--rus-logo-disc-size',`${disc.size||72}px`)}wrap.appendChild(addImg(name,'rus-card-logo'));content.insertBefore(wrap,content.firstChild)});
