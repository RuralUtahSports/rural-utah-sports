...
async function bounded(task,timeout=1200,fallback=null){let timer;try{return await Promise.race([Promise.resolve().then(task),new Promise(resolve=>{timer=setTimeout(()=>resolve(fallback),timeout)})])}catch{return fallback}finally{if(timer)clearTimeout(timer)}}
async function retireServiceWorkers(){
  try{
    let hadController=false;
    if('serviceWorker' in navigator){
      hadController=!!navigator.serviceWorker.controller;
      const registrations=await bounded(()=>navigator.serviceWorker.getRegistrations(),1200,[]);
      await bounded(()=>Promise.allSettled((registrations||[]).map(registration=>registration.unregister())),1200,[]);
    }
    ...
    if(hadController){
      let alreadyReloaded=false,canTrack=false;
      try{alreadyReloaded=sessionStorage.getItem('rus-sw-retired-reload')==='1';canTrack=true;if(!alreadyReloaded)sessionStorage.setItem('rus-sw-retired-reload','1')}catch{}
      if(!alreadyReloaded&&canTrack){
