...
function markup(){return `<section class="rus-elo-info" aria-label="What is ELO?">
  ...
  <p class="rus-elo-info-lead"><strong>ELO is a running strength rating, not a poll.</strong> Every program begins at 1500 in the RUS model, then its rating moves after each recorded game based mainly on the result and the opponent's rating. RUS also applies a small, capped margin-of-victory bonus so dominant wins carry a little more weight without letting blowouts take over the system.</p>
  <div class="rus-elo-info-grid">
    ...
    <div class="rus-elo-info-item"><strong>Opponent Strength Matters Most</strong><span>Beating a higher-rated opponent earns more. Beating a much lower-rated opponent earns fewer points.</span></div>
    ...
    <div class="rus-elo-info-item"><strong>Capped Margin Bonus</strong><span>The normal ELO change is multiplied by a mild logarithmic margin bonus, capped at 1.35× once the margin reaches 40 points.</span></div>
  </div>
  <p class="rus-elo-example"><b>Approximate margin multipliers:</b> 1 point = 1.00× • 7 points ≈ 1.13× • 21 points ≈ 1.26× • 40+ points = 1.35×.</p>
  <p class="rus-elo-formula"><b>RUS settings:</b> K-factor 32 • Win = 1 • Tie = 0.5 • Loss = 0 • margin multiplier = 1 + 0.35 × (ln(min(margin, 40)) ÷ ln(40))<sup>1.5</sup> • ratings are generated from the deduplicated master game database.</p>
</section>`}
function insertMain(){
