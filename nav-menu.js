(()=>{
  ...
  function loadExtras(){
    ...
    if(path==='team.html'&&!document.querySelector('script[data-rus-team-schedule-reconcile]')){const s=document.createElement('script');s.src='team-schedule-reconcile.js?v=20260813a';s.dataset.rusTeamScheduleReconcile='1';document.body.appendChild(s)}
  }
  function setup(){
    injectStyles();const host=document.querySelector('nav .nav-content');if(!host){loadExtras();return}host.classList.add('rus-nav');host.innerHTML=[link('Home','index.html','home-link'),link('Teams','teams.html'),link('Games','games.html'),link('Scoreboard','scoreboard.html'),link('Rankings','rankings.html'),link('Standings','standings.html'),link('Storylines','storylines.html'),dropdown('History','history'),dropdown('Analytics','analytics'),dropdown('Simulators','simulators')].join('');
