...
function orderHref(entry){
  ...
  if(!email)return'';
  return`mailto:${email}?subject=${encodeURIComponent(`${entry.business||'Sponsor'} order inquiry`)}`;
}
function styles(){
...
function renderSponsored(entry,hero){
  ...
  const logo=logoImg?(mail?`<a class="rus-team-hero-sponsor-logo-link" href="${esc(mail)}" aria-label="Email ${esc(entry.business||'sponsor')} to submit an order">${logoImg}</a>`:logoImg):'';
