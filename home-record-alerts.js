...
const ORANGE='#F14D07',BLACK='#050505',WHITE='#FFFFFF',MUTED='#A8A8A8';
const compact=v=>String(v??'').trim().toUpperCase().replace(/[^A-Z0-9]/g,'');
const aliases={CEDAR:'CEDARCITY',CEDARCITY:'CEDARCITY',GRANDCOUNTY:'GRAND',GUNNISON:'GUNNISONVALLEY',MONUMENTVAL:'MONUMENTVALLEY',MAPLEMTN:'MAPLEMOUNTAIN',AMERICANLEADERSHIPACADEMY:'ALA',AMERICANLEADERSHIP:'ALA'};
const canon=v=>aliases[compact(v)]||compact(v);
const teamMeta=new Map();
function validHex(v,fallback){return /^#[0-9a-f]{6}$/i.test(String(v||''))?String(v):fallback}
function hexRgb(hex){const h=validHex(hex,'#777777').slice(1);return[parseInt(h.slice(0,2),16),parseInt(h.slice(2,4),16),parseInt(h.slice(4,6),16)]}
function readableAccent(hex){const [r,g,b]=hexRgb(hex),lum=(.2126*r+.7152*g+.0722*b)/255;if(lum>=.38)return hex;const mix=x=>Math.round(x+(255-x)*.38).toString(16).padStart(2,'0');return`#${mix(r)}${mix(g)}${mix(b)}`}
function themeFor(team){const m=teamMeta.get(canon(team))||{},bg=validHex(m.backgroundColor,ORANGE),fg=validHex(m.textColor,WHITE);return{bg,fg,accent:readableAccent(bg)}}
async function loadTeamMeta(){try{const r=await fetch(`teams-data.json?v=${Date.now()}`,{cache:'no-store'});if(!r.ok)return;const rows=await r.json();if(!Array.isArray(rows))return;for(const t of rows)teamMeta.set(canon(t.team),t)}catch(e){console.warn('Record alert team colors:',e)}}
...
function weekStart(t){const d=new Date(t);d.setHours(0,0,0,0);d.setDate(d.getDate()-d.getDay());return d.getTime()}
...
.rus-record-alert-heading strong{color:#F14D07}
.rus-record-alert-shareall{border:1px solid #F14D07;background:#F14D07;color:#000;border-radius:5px;font-size:8px;font-weight:1000;text-transform:uppercase;padding:6px 8px;cursor:pointer;white-space:nowrap}
.rus-record-alert-scroller{display:flex;overflow-x:auto;overscroll-behavior-x:contain;scroll-snap-type:x mandatory;scrollbar-width:none;-webkit-overflow-scrolling:touch;touch-action:pan-x pan-y}
.rus-record-alert-scroller::-webkit-scrollbar{display:none}
...
function downloadFiles(files){files.forEach((x,i)=>setTimeout(()=>{const a=document.createElement('a');a.href=x.url;a.download=x.file.name;document.body.appendChild(a);a.click();a.remove()},i*180))}
