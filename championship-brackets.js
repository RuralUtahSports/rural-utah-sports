...
function teamHTML(label,score,isWin){const t=lookup(label),bg=t?safeHex(t.backgroundColor,'#F14D07'):null,fg=t?safeHex(t.textColor,'#FFFFFF'):'#FFFFFF';const css=bg?` style="--team-bg:${bg};--team-fg:${fg}"`:'';const name=clean(label);const linked=t?`<a class="bracket-team-name" href="team.html?team=${encodeURIComponent(t.team)}" title="View ${stripSeed(name)} team page">${name}</a>`:`<span class="bracket-team-name">${name}</span>`;return `<div class="bracket-team ${isWin?'win ':''}${bg?'has-color':''}"${css}>${linked}<span class="bracket-score ${isWin?'win-score':''}">${score}</span></div>`}
