...
function gameHTML(g){const a=nScore(g[1]),b=nScore(g[3]);const aw=a!==null&&b!==null?a>b:(String(g[3]).toUpperCase()==='F');const bw=a!==null&&b!==null?b>a:false;return `<div class="bracket-game"><div class="bracket-team ${aw?'win':''}"><span>${g[0]}</span><span class="bracket-score">${g[1]}</span></div><div class="bracket-team ${bw?'win':''}"><span>${g[2]}</span><span class="bracket-score">${g[3]}</span></div></div>`}
function render(cls){[...tabs.children].forEach(b=>b.classList.toggle('active',b.dataset.cls===cls));grid.innerHTML=data[cls].map((r,i)=>`<div class="bracket-round ${i===data[cls].length-1?'bracket-champ':''}"><div class="bracket-round-title">${r[0]}</div><div class="bracket-games">${r[1].map(gameHTML).join('')}</div></div>`).join('');sec.querySelector('.bracket-scroll').scrollLeft=0}
Object.keys(data).forEach((cls,i)=>{const b=document.createElement('button');b.type='button';b.className='bracket-tab'+(i===0?' active':'');b.dataset.cls=cls;b.textContent=cls;b.addEventListener('click',()=>render(cls));tabs.appendChild(b)});render('6A');
})();