...
const featureSponsors=()=>window.RUSFeatureSponsorsPromise||(window.RUSFeatureSponsorsPromise=fetch(`feature-sponsors.json?v=${Date.now()}`,{cache:'no-store'}).then(r=>r.ok?r.json():{}).catch(()=>({})));
...
async function start(){installRankingExportCardFix();installExportInterceptor();document.querySelectorAll('.rus-export-board').forEach(patchExportBoard);try{const cfg=await featureSponsors(),s=cfg?.stateTop25;if(s?.mode==='sponsor')return;try{delete window.RUSState25FeatureSponsor}catch{window.RUSState25FeatureSponsor=null}installCanvasMask();removeSponsorNodes();const o=new MutationObserver(removeSponsorNodes);o.observe(document.body,{childList:true,subtree:true});setTimeout(()=>{removeSponsorNodes();o.disconnect()},30000)}catch(e){console.warn('State Top 25 sponsor removal unavailable',e)}}
