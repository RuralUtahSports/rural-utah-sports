...
async function run(){
  ...
  const wins=all.filter(x=>x.type==='W'&&x.len>=11).sort((a,b)=>b.len-a.len||a.team.localeCompare(b.team)).slice(0,30);
  const losses=all.filter(x=>x.type==='L'&&x.len>=11).sort((a,b)=>b.len-a.len||a.team.localeCompare(b.team)).slice(0,20);
