...
function apply(a,h){const paint=()=>document.querySelectorAll('.rus-gc-panel').forEach(panel=>{panel.style.setProperty('--rus-away',a);panel.style.setProperty('--rus-home',h);panel.style.setProperty('--rus-away-soft',rgba(a,.13));panel.style.setProperty('--rus-home-soft',rgba(h,.13))});paint();const page=document.getElementById('page')||document.body,o=new MutationObserver(paint);o.observe(page,{childList:true,subtree:true});setTimeout(()=>o.disconnect(),15000)}
