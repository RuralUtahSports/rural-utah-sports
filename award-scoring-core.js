(function(root,factory){
  ...
  const QUALITY_WIN_RECORD_WEIGHT=.60;
  const QUALITY_WIN_TOP25_WEIGHT=.40;
  ...
  function buildTeamContexts(standings,rankings){
    ...
    const raw=[...records.entries()].map(([key,record])=>{
      ...
      for(const game of played)if(game.won){
        let strength=qualityWinOpponentStrength(records,top25Ranks,game.opponent);
        const rank=top25Ranks.get(game.opponent);
        if(rank&&rank<=5)strength+=.25;
        else if(rank&&rank<=10)strength+=.15;
        else if(rank&&rank<=25)strength+=.05;
        qualityWinTotal+=strength;qualityWinCount++;if(rank)top25Wins++;
      }
