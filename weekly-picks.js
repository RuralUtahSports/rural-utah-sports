...
function wpCurrentFootballWeekKey(p){
  ...
  if(day===0&&wpMountainParts().h>=8)d.setUTCDate(d.getUTCDate()+4);
  else if(day>=1&&day<=3)d.setUTCDate(d.getUTCDate()+(4-day));
...
// Prediction visibility is independent of the visitor pick deadline.
function wpPredictionReleaseDate(w){const p=wpParseDate(w.key)||w.firstDate,d=new Date(Date.UTC(p.y,p.m-1,p.d));d.setUTCDate(d.getUTCDate()-d.getUTCDay());return{y:d.getUTCFullYear(),m:d.getUTCMonth()+1,d:d.getUTCDate()}}
function wpPredictionsReleased(w){return wpMountainNowKey()>=wpYmdKey(wpPredictionReleaseDate(w))*10000+800}
function wpPredictionReleaseLabel(w){return `${wpDateLabel(wpPredictionReleaseDate(w))} at 8:00 AM Mountain Time`}
