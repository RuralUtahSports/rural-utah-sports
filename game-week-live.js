...
function card(g,map){const isFinal=final(g),pa=num(g.awayScore),ph=num(g.homeScore),aa=num(g.actualAway),ah=num(g.actualHome);let score='',label='Scheduled';if(isFinal){score=`${aa}-${ah}`;label='Final'}else if(pa!==null&&ph!==null){score=`${pa}-${ph}`;label='RUS projection'}return `<article class="rus-week-card"><div class="rus-week-top"><div class="rus-week-matchup">${teamPill(g.awayTeam,map)}<span class="at">at</span>${teamPill(g.homeTeam,map)}</div>${score?`<div class="rus-week-score ${isFinal?'':'projected'}">${esc(score)}</div>`:''}</div><div class="rus-week-bottom"><div class="rus-week-meta">${esc(fmtDate(g.date))} • ${label}</div><a class="rus-week-view" href="${gameHref(g)}">View Game →</a></div></article>`}
function section(title,games,map){return `<h2 class="section-title">${esc(title)}</h2>${games.length?`<div class="rus-week-grid">${games.map(g=>card(g,map)).join('')}</div>`:'<div class="rus-week-empty">No games to show here yet.</div>'}`}
async function run(){
 ...
 for(const [cls,arr] of Object.entries(rankingGroups))(arr||[]).forEach((x,i)=>rankMap.set(norm(typeof x==='string'?x:x.team),{cls,rank:i+1}));
