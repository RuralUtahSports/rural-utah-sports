...
async function assets(){
  ...
  if(!script){script=document.createElement('script');script.src='school-assets-core.js?v=20260818-perf4';script.async=true;document.head.appendChild(script)}
  for(let i=0;i<75&&!window.RUSSchoolAssets;i++)await new Promise(r=>setTimeout(r,40));
...
async function insert(){
  ...
  let triedFallback=false;img.addEventListener('error',()=>{if(triedFallback)return;triedFallback=true;const fallback=api.fallbackLogo?.(team)||'';if(fallback&&srcKey(fallback)!==srcKey(img.src))img.src=fallback},{once:false});
  const copy=document.createElement('div');copy.className='rus-team-brand-copy';title.parentNode.insertBefore(wrap,title);wrap.append(img,copy);copy.append(title,subtitle);dedupe(hero,img);
  return true;
}
insert().then(ok=>{
  if(ok)return;
  const root=document.getElementById('page');if(!root)return;
  const observer=new MutationObserver(async()=>{if(await insert())observer.disconnect()});observer.observe(root,{childList:true,subtree:true});setTimeout(()=>observer.disconnect(),10000);
});
})();
