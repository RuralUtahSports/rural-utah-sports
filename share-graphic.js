(()=>{
  ...
  const PAGE=(location.pathname.split('/').pop()||'').toLowerCase(),IS_HOME=PAGE==='index.html'||PAGE==='';
  if(!/award|standings|scoreboard|rankings/.test(PAGE)&&!IS_HOME)return;
  ...
  async function make(format,target=null,labelOverride='',preparedBlob=null){
    ...
    const blob=preparedBlob||(weeklyTarget?await renderWeeklyTarget(el,dims[0],dims[1],label):reviewTarget?await renderWeekReviewTarget(el,dims[0],dims[1],label):await render(el,dims[0],dims[1],label));await deliver(blob,`rural-utah-sports-${format}-${Date.now()}.png`);
  }
  async function modal(initialTarget=null,initialLabel=''){
    ...
    const collectionTarget=initialTarget?.matches?.('.class-grid'),reviewCollection=initialTarget?.matches?.('.week-review-grid'),reviewCard=initialTarget?.matches?.('.review-card'),el=document.createElement('div');el.className='rus-share-modal';el.innerHTML=`<div class="rus-share-sheet"><h3>Share Graphic</h3><p>${PAGE.includes('standings')?'Share one region, or choose a whole classification to put every region in that class on one graphic using the same region-by-region standings look.':collectionTarget?'Creates one branded graphic featuring all seven classification MVPs.':reviewCollection?'Creates one branded graphic with every available Week in Review highlight.':reviewCard?'Creates a branded graphic for this Week in Review highlight.':initialTarget?'Creates a branded graphic for this weekly award using the player’s school colors, logo, stats and result.':'Creates a branded graphic from the section currently in view. Rankings use team colors, local logos and a layout that fills the canvas.'}</p>${standingsControls}<div class="rus-share-grid"><button class="rus-share-option" data-f="square"><strong>Instagram Post</strong>1080 × 1080</button><button class="rus-share-option" data-f="story"><strong>Instagram Story</strong>1080 × 1920</button><button class="rus-share-option" data-f="x"><strong>X Post</strong>1600 × 900</button><button class="rus-share-option" data-f="square"><strong>Square PNG</strong>Download / Share</button></div><button class="rus-share-close">Cancel</button></div>`;
    document.body.appendChild(el);
    const preparedDirect=new Map(),weeklyShare=PAGE==='weekly-awards.html'&&initialTarget?.matches?.('.award-card,.class-card,.class-grid'),reviewShare=IS_HOME&&initialTarget?.matches?.('.review-card,.week-review-grid');
    if(weeklyShare||reviewShare){
      ...
      [...new Set(buttons.map(button=>button.dataset.f))].forEach(async format=>{const dims=format==='story'?[1080,1920]:format==='x'?[1600,900]:[1080,1080];try{const blob=reviewShare?await renderWeekReviewTarget(initialTarget,dims[0],dims[1],initialLabel||titleFor(initialTarget)):await renderWeeklyTarget(initialTarget,dims[0],dims[1],initialLabel||titleFor(initialTarget));preparedDirect.set(format,blob);buttons.filter(button=>button.dataset.f===format).forEach(button=>{button.disabled=false;button.innerHTML=original.get(button)})}catch(error){console.error(error);buttons.filter(button=>button.dataset.f===format).forEach(button=>{button.disabled=false;button.innerHTML='<strong>Try Again</strong>Tap to retry'})}});
    }
  ...
  function init(){if(PAGE==='weekly-awards.html')return;if(IS_HOME){document.getElementById('shareWeekReview')?.addEventListener('click',()=>{const target=document.getElementById('weekReviewGrid');if(!target?.querySelector('[data-review-card]'))return alert('Week in Review highlights are still loading.');modal(target,'2026 Football • Week in Review')});document.addEventListener('click',event=>{const button=event.target.closest?.('.review-share-button');if(!button)return;const target=button.closest('.review-card'),label=(target?.querySelector('.review-label')?.textContent||'Week in Review').trim();modal(target,`${label} • Week in Review`)});return}const b=document.createElement('button');b.className='rus-share-btn rus-share-float';b.textContent='Share Graphic';b.onclick=()=>modal();document.body.appendChild(b)}
