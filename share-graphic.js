(()=>{
  ...
  async function renderStatLeaderAll(w,h){
    const data=window.RUSStatLeadersShareAllData?.();
    if(!data?.sections?.length)throw new Error('Stat leaders are still loading');
    const story=h>w*1.3,landscape=w>h*1.25,limit=story?5:3,sections=data.sections.map(section=>({...section,players:(section.players||[]).slice(0,limit)})).filter(section=>section.players.length&&section.metrics?.length);
    if(!sections.length)throw new Error('No reported stat leaders are available');
    const out=document.createElement('canvas');out.width=w;out.height=h;const c=out.getContext('2d'),bg=c.createLinearGradient(0,0,w,h);bg.addColorStop(0,'#050505');bg.addColorStop(.72,'#111');bg.addColorStop(1,'#080808');c.fillStyle=bg;c.fillRect(0,0,w,h);c.fillStyle='#F14D07';c.fillRect(0,0,w,12);
    const margin=story?42:landscape?50:40,titleY=story?62:landscape?58:60;c.fillStyle='#fff';c.textAlign='left';c.textBaseline='alphabetic';fitCanvasFont(c,'STAT LEADERS',w-margin*2,landscape?44:42,27,900);c.fillText('STAT LEADERS',margin,titleY);c.fillStyle='#F14D07';fitCanvasFont(c,`${data.season} • ${String(data.scope).toUpperCase()} • ${String(data.classification).toUpperCase()}`,w-margin*2,landscape?18:19,11,900);c.fillText(`${data.season} • ${String(data.scope).toUpperCase()} • ${String(data.classification).toUpperCase()}`,margin,titleY+31);c.fillStyle='#777';fitCanvasFont(c,`ALL TRACKED CATEGORIES • TOP ${limit} BY CATEGORY`,w-margin*2,landscape?14:15,10,900);c.fillText(`ALL TRACKED CATEGORIES • TOP ${limit} BY CATEGORY`,margin,titleY+58);
    const cols=story?1:2,contentTop=story?142:landscape?128:132,footerH=story?37:34,gap=story?10:8,rows=Math.ceil(sections.length/cols),cardW=(w-margin*2-gap*(cols-1))/cols,cardH=(h-contentTop-footerH-gap*(rows-1))/rows;
    const drawSection=(section,x,y,width,height)=>{
      const pad=story?14:11,titleH=story?36:30,headerH=story?28:24,metrics=section.metrics||[],rankW=story?34:27,playerW=Math.min(width*.4,story?365:landscape?300:220),metricW=(width-pad*2-rankW-playerW)/metrics.length,tableY=y+titleH,rowH=Math.max(18,(height-titleH-headerH-pad)/section.players.length);
      c.fillStyle='#080808';c.strokeStyle='#373737';c.lineWidth=2;canvasRoundRect(c,x,y,width,height,12);c.fill();c.stroke();c.fillStyle='#F14D07';canvasRoundRect(c,x,y,7,height,4);c.fill();c.fillStyle='#F14D07';c.textAlign='left';c.textBaseline='alphabetic';fitCanvasFont(c,section.category.toUpperCase(),width-pad*2-(story?190:135),story?19:landscape?15:14,9,900);c.fillText(section.category.toUpperCase(),x+pad,y+22);c.fillStyle='#777';fitCanvasFont(c,`BY ${String(section.rankMetric||'STAT').toUpperCase()}`,width-pad*2-(story?190:135),story?12:landscape?10:9,7,900);c.textAlign='right';c.fillText(`BY ${String(section.rankMetric||'STAT').toUpperCase()}`,x+width-pad,y+21);
      c.fillStyle='#F14D07';canvasRoundRect(c,x+pad,tableY,width-pad*2,headerH,5);c.fill();c.fillStyle='#000';c.textBaseline='middle';canvasFont(c,story?10:8,900);c.textAlign='center';c.fillText('#',x+pad+rankW/2,tableY+headerH/2);c.textAlign='left';fitCanvasFont(c,'PLAYER / TEAM',playerW-10,story?10:8,6,900);c.fillText('PLAYER / TEAM',x+pad+rankW+7,tableY+headerH/2);metrics.forEach((label,index)=>{const cellX=x+pad+rankW+playerW+metricW*index;fitCanvasFont(c,String(label).toUpperCase(),metricW-5,story?10:8,6,900);c.textAlign='center';c.fillText(String(label).toUpperCase(),cellX+metricW/2,tableY+headerH/2)});
      section.players.forEach((player,index)=>{const rowY=tableY+headerH+index*rowH,color=/^#[0-9a-f]{6}$/i.test(String(player.teamColor||''))?player.teamColor:'#555555';c.fillStyle=index%2?'#0b0b0b':'#141414';c.fillRect(x+pad,rowY,width-pad*2,rowH);c.fillStyle=rgba(color,.17);c.fillRect(x+pad,rowY,rankW+playerW,rowH);c.fillStyle=color;c.fillRect(x+pad,rowY,5,rowH);c.strokeStyle='#2e2e2e';c.lineWidth=1;c.beginPath();c.moveTo(x+pad,rowY+rowH);c.lineTo(x+width-pad,rowY+rowH);c.stroke();c.fillStyle=index<3?'#F14D07':'#fff';canvasFont(c,story?17:14,900);c.textAlign='center';c.textBaseline='middle';c.fillText(String(index+1),x+pad+rankW/2,rowY+rowH/2);const copyX=x+pad+rankW+7,copyW=playerW-14,nameY=rowY+rowH*.43,metaY=rowY+rowH*.77;c.fillStyle='#fff';fitCanvasFont(c,player.name,copyW,story?17:landscape?14:12,8,900);c.textAlign='left';c.textBaseline='alphabetic';c.fillText(String(player.name),copyX,nameY);const meta=`${player.number?`#${player.number} • `:''}${String(player.team||'').toUpperCase()} • ${String(player.classification||'').toUpperCase()}`;c.fillStyle=readableAccent(color);fitCanvasFont(c,meta,copyW,story?11:landscape?9:8,7,900);c.fillText(meta,copyX,metaY);metrics.forEach((label,metricIndex)=>{const cellX=x+pad+rankW+playerW+metricW*metricIndex,id=String(label||'').toUpperCase().replace(/[^A-Z0-9]/g,'');if(id===String(section.rankMetric||'').toUpperCase().replace(/[^A-Z0-9]/g,'')){c.fillStyle='rgba(241,77,7,.1)';c.fillRect(cellX,rowY,metricW,rowH)}c.fillStyle=id===String(section.rankMetric||'').toUpperCase().replace(/[^A-Z0-9]/g,'')?'#F14D07':'#f1f1f1';fitCanvasFont(c,String(player.values?.[label]??'—'),metricW-6,story?17:landscape?13:11,7,900);c.textAlign='center';c.textBaseline='middle';c.fillText(String(player.values?.[label]??'—'),cellX+metricW/2,rowY+rowH/2)});});
    };
    sections.forEach((section,index)=>{const row=Math.floor(index/cols),col=index%cols,count=Math.min(cols,sections.length-row*cols),total=count*cardW+(count-1)*gap,x=(w-total)/2+col*(cardW+gap);drawSection(section,x,contentTop+row*(cardH+gap),cardW,cardH)});
    c.fillStyle='#777';canvasFont(c,story?12:11,800);c.textAlign='left';c.textBaseline='alphabetic';c.fillText(`REPORTED STATS • MISSING FIELDS SHOWN AS — • TOP ${limit} BY CATEGORY`,margin,h-18);c.fillStyle='#999';canvasFont(c,story?13:12,900);c.textAlign='right';c.fillText('@ruralutahsports77',w-margin,h-18);
    const blob=await new Promise(resolve=>out.toBlob(resolve,'image/png',1));if(!blob)throw new Error('PNG export failed');return blob;
  }
  ...
  async function make(format,target=null,labelOverride='',preparedBlob=null,statMode='category'){
    ...
    const statAllTarget=IS_STAT_LEADERS&&statMode==='all'&&el?.id==='results',statTarget=IS_STAT_LEADERS&&!statAllTarget&&el?.id==='results';
    const blob=preparedBlob||(weeklyTarget?await renderWeeklyTarget(el,dims[0],dims[1],label):reviewTarget?await renderWeekReviewTarget(el,dims[0],dims[1],label):statAllTarget?await renderStatLeaderAll(dims[0],dims[1]):statTarget?await renderStatLeaderCategory(dims[0],dims[1]):await render(el,dims[0],dims[1],label));await deliver(blob,`rural-utah-sports-${format}-${Date.now()}.png`);
  }
  async function modal(initialTarget=null,initialLabel='',statMode='category'){
    ...
    const collectionTarget=initialTarget?.matches?.('.class-grid'),reviewCollection=initialTarget?.matches?.('.week-review-grid'),reviewCard=initialTarget?.matches?.('.review-card'),statAllTarget=IS_STAT_LEADERS&&statMode==='all'&&initialTarget?.id==='results',statTarget=IS_STAT_LEADERS&&!statAllTarget&&initialTarget?.id==='results',el=document.createElement('div');el.className='rus-share-modal';el.innerHTML=`<div class="rus-share-sheet"><h3>${statAllTarget?'Share All Stats':'Share Graphic'}</h3><p>${PAGE.includes('standings')?'Share one region, or choose a whole classification to put every region in that class on one graphic using the same region-by-region standings look.':collectionTarget?'Creates one branded graphic featuring all seven classification MVPs.':reviewCollection?'Creates one branded graphic with every available Week in Review highlight.':reviewCard?'Creates a branded graphic for this Week in Review highlight.':statAllTarget?'Combines every tracked stat category and metric into one board. The portrait version includes the top five players per category; shorter formats use the top three so the numbers stay readable.':statTarget?'Creates one leaderboard graphic with the top reported players and every tracked metric for this category and filter.':initialTarget?'Creates a branded graphic for this weekly award using the player’s school colors, logo, stats and result.':'Creates a branded graphic from the section currently in view. Rankings use team colors, local logos and a layout that fills the canvas.'}</p>${standingsControls}<div class="rus-share-grid"><button class="rus-share-option" data-f="square"><strong>Instagram Post</strong>1080 × 1080</button><button class="rus-share-option" data-f="story"><strong>Instagram Story</strong>1080 × 1920</button><button class="rus-share-option" data-f="x"><strong>X Post</strong>1600 × 900</button><button class="rus-share-option" data-f="square"><strong>Square PNG</strong>Download / Share</button></div><button class="rus-share-close">Cancel</button></div>`;
    document.body.appendChild(el);
    const preparedDirect=new Map(),weeklyShare=PAGE==='weekly-awards.html'&&initialTarget?.matches?.('.award-card,.class-card,.class-grid'),reviewShare=IS_HOME&&initialTarget?.matches?.('.review-card,.week-review-grid'),statAllShare=statAllTarget,statShare=IS_STAT_LEADERS&&statTarget;
    if(weeklyShare||reviewShare||statAllShare||statShare){
    ...
    el.querySelectorAll('[data-f]').forEach(b=>b.onclick=async()=>{const f=b.dataset.f;b.disabled=true;b.textContent='Creating…';try{let target=initialTarget,label=initialLabel;if(PAGE.includes('standings')){const shareScope=el.querySelector('.rus-share-standings-scope')?.value||'region';if(shareScope==='classification'){
        ...
        }}await make(f,target,label,preparedDirect.get(f)||null,statMode);el.remove()}catch(err){console.error(err);b.disabled=false;b.textContent='Try Again';alert('Could not create the graphic. Please try again.')}});
  }
  Object.assign(window.RUSShareGraphic,{openModal:modal,make});
  function init(){if(PAGE==='weekly-awards.html')return;if(IS_STAT_LEADERS){document.getElementById('shareStatCategory')?.addEventListener('click',()=>{const data=window.RUSStatLeadersShareData?.();if(!data?.players?.length)return alert('Stat leaders are still loading for this filter.');modal(document.getElementById('results'),`${data.category} Leaders • ${data.season} ${data.scope}`)});document.getElementById('shareAllStats')?.addEventListener('click',()=>{const data=window.RUSStatLeadersShareAllData?.();if(!data?.sections?.length)return alert('Stat leaders are still loading for this filter.');modal(document.getElementById('results'),`All Stat Leaders • ${data.season} ${data.scope}`,'all')});return}if(IS_HOME){document.getElementById('shareWeekReview')?.addEventListener('click',()=>{const target=document.getElementById('weekReviewGrid');if(!target?.querySelector('[data-review-card]'))return alert('Week in Review highlights are still loading.');modal(target,'2026 Football • Week in Review')});document.addEventListener('click',event=>{const button=event.target.closest?.('.review-share-button');if(!button)return;const target=button.closest('.review-card'),label=(target?.querySelector('.review-label')?.textContent||'Week in Review').trim();modal(target,`${label} • Week in Review`)});return}if(PAGE==='scoreboard.html'&&document.getElementById('shareScoreboardGrid'))return;const b=document.createElement('button');b.className='rus-share-btn rus-share-float';b.textContent='Share Graphic';b.onclick=()=>modal();document.body.appendChild(b)}
