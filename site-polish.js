...
function setupPickemNav(){
  ...
  const drop=details?.querySelector('.drop');if(!drop||drop.querySelector('a[href="simulators.html?tab=weekly"]'))return;
  const a=document.createElement('a');a.href='simulators.html?tab=weekly';a.textContent='Weekly Pick’em';drop.appendChild(a);
}
