(()=>{
'use strict';
if(window.__RUSSchoolColorsPageV2)return;window.__RUSSchoolColorsPageV2=true;
...
function addStyles(){
  ...
  const s=document.createElement('style');s.id='rus-school-color-style';s.textContent=`
...
function colorVars(el,t){
  ...
  el.style.setProperty('--rus-school-color',bg);el.style.setProperty('--rus-school-text',fg);return {bg,fg};
}
function paintLink(a,name){
  const t=lookup(name||a.textContent);if(!t)return;
  const {bg,fg}=colorVars(a,t);a.classList.add('rus-team-color');a.style.backgroundColor=bg;a.style.color=fg;
...
function paintPrograms(){document.querySelectorAll('#page td.team a').forEach(a=>paintLink(a))}
function paintSeason(){
  ...
  document.querySelectorAll('#page .champ').forEach(card=>{
    ...
    const name=clean(h.textContent),t=lookup(name);if(!t)return;
...
function paintHomeDataTeams(){
  document.querySelectorAll('[data-team]').forEach(el=>{
    const t=lookup(el.getAttribute('data-team'));if(!t)return;const {bg,fg}=colorVars(el,t);
    if(el.classList.contains('watch-team')||el.classList.contains('watch-badge')){el.style.setProperty('--team-bg',bg);el.style.setProperty('--team-fg',fg)}
  });
}
function paintHomeRecordWatch(){
  document.querySelectorAll('.rus-watch-card').forEach(card=>{
    let team='';try{team=new URL(card.getAttribute('href')||'',location.href).searchParams.get('team')||''}catch{}
    const strong=card.querySelector('strong'),t=lookup(team);if(!strong||!t)return;colorVars(strong,t);strong.classList.add('rus-home-single-team');
  });
}
function paintHomeHistory(){
  document.querySelectorAll('.rus-history-card').forEach(card=>{
    if(card.dataset.rusHomeColors==='1')return;
    let u;try{u=new URL(card.getAttribute('href')||'',location.href)}catch{return}
    ...
    strong.innerHTML=`${chip(a)}<span class="rus-history-score">${score}</span>${chip(b)}`;card.dataset.rusHomeColors='1';
  });
}
function paintHomeSearch(){
  document.querySelectorAll('.rus-search-item').forEach(item=>{
    let team='';try{const u=new URL(item.getAttribute('href')||'',location.href);if((u.pathname.split('/').pop()||'').toLowerCase()==='team.html')team=u.searchParams.get('team')||''}catch{}
    const strong=item.querySelector('strong'),t=lookup(team);if(!strong||!t)return;colorVars(strong,t);strong.classList.add('rus-home-single-team');
  });
}
...
async function init(){
  addStyles();
  const pageRoot=document.getElementById('page')||document.querySelector('main')||document.body;
  if(path==='championships.html'){
    suppressTitleCount();new MutationObserver(suppressTitleCount).observe(pageRoot,{childList:true,subtree:true});return;
  }
  try{
    const r=await fetch('teams-data.json');
  ...
  paint();
  let scheduled=false;
  new MutationObserver(()=>{if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;paint()})}).observe(pageRoot,{childList:true,subtree:true});
}
